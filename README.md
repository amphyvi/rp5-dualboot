...
* **Rocknix is looking for GameCube games in the "gc" folder - but on Android, ES-DE is looking in the "gamecube" folder. Help!** Yeah... for some reason, Rocknix ES & Android ES-DE look in different folders for certain systems, namely GameCube. The solution is a bit techie, but it works:
 1. Uninstall ES-DE from Android & delete the ES-DE folder from the `STORAGE` partition of the SD card.
 2. Follow the guide & set up Rocknix the first time.
 3. Download the only XML file (`es-systems.xml`) in this GitHub repo (thanks, [Jade](https://github.com/GlazedBelmont)!)
 4. On your device: Go to the `STORAGE` partition of the SD card, then your ES-DE folder, then open the `custom_systems` folder. Put the XML file here.
 5. Put all your GameCube games in the `/games-internal/roms/gamecube` folder. (If the `gamecube` folder doesn't exist for some reason, create it!)
 6. Install ES-DE on Android. During the setup process, be sure to point it to `/games-internal/roms`. When prompted to "Create system directories", press Skip - Rocknix already did that. That should do the trick!
